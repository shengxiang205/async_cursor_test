...
hosts             = %w{ mongoone.joowing.com:40000 mongotwo.joowing.com:40000 mongothree.joowing.com:40000 }
$mongo_connection = Mongo::MongoReplicaSetClient.new(hosts, :pool_size => 64)
$db               = $mongo_connection['data_ocean_production_test_env']
...
module Async
  ...
  class AsyncCursor
    ...
    def checkout_em_socket(&blk)
      ...
      self.class.pool(host, port).checkout do |s|
        blk.call(s)
      end
    end
...
class Api
  def self.call(env)
    ...
    if request.path =~ /profile/
      ...
      cursor = $db['cache_processor_droplets'].find(
